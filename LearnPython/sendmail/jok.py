...
import urllib2, json, sys, os, time, random
...
class BaseImg(object):
    def __init__(self, url):
        ...
        self.uri = ("data:" + response.headers['Content-Type'] + ";" + "base64," + base64.b64encode(response.content))
...
class Jok(object):
    ...
    def text_jok(self):
        ...
        con = json.load(urllib2.urlopen(url))['showapi_res_body']['contentlist']
        num = int(random.uniform(0, len(con)))
        return con[num]['text']

    def base_img(self, url):
        response = requests.get(url)
        basecon = ("data:" + response.headers['Content-Type'] + ";" + "base64," + base64.b64encode(response.content))
        return basecon
